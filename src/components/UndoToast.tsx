...
export function UndoToast({ message, duration = 5000, onUndo, onExpire }: UndoToastProps) {
  ...
  return (
    <div className="fixed top-safe left-0 right-0 z-50 flex justify-center px-4 pt-4 animate-toast-in" role="status" aria-live="polite">
      <div className="bg-ct-surface border border-ct-border rounded-ct-lg px-4 py-3 shadow-xl flex items-center gap-3 max-w-sm w-full">
        <p className="text-sm text-ct-1 flex-1">{message}</p>
        <button onClick={onUndo}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan-500/15 text-cyan-400 text-sm font-bold active:bg-cyan-500/25 shrink-0">
          <Undo2 size={14} /> Undo
        </button>
        {/* Progress bar */}
