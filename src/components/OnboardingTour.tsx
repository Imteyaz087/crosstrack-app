...
export function OnboardingTour() {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-end justify-center" onClick={dismiss}
      ...
      {/* Card */}
      <div
        className={`relative mx-4 mb-24 max-w-sm w-full bg-slate-800 rounded-ct-lg border border-slate-700 p-5 shadow-2xl animate-fade-in ${
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-1.5">
            {STEPS.map((_, i) => (
              <div key={i} className={`w-2 h-2 rounded-full transition-colors ${i === step ? 'bg-cyan-400' : i < step ? 'bg-cyan-400/40' : 'bg-slate-600'}`} />
            ))}
          </div>
          ...
          <div className="flex gap-2">
            {!isFirst && (
              <button onClick={() => goToStep(step - 1)}
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-ct-elevated text-ct-2 text-xs font-medium active:bg-slate-600">
