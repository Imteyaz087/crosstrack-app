...
export function GroceryPage() {
  ...
  useEffect(() => { loadGrocery().finally(() => setLoading(false)) }, [])
  ...
  return (
    <div className="space-y-4 stagger-children">
      <div className="flex justify-between items-center">
        ...
        <button onClick={resetGrocery} className="text-ct-2 flex items-center gap-1 text-xs p-2 min-h-[44px] rounded-lg active:bg-ct-elevated/50" aria-label="Reset all grocery items">
          ...
          <div key={cat} className="bg-ct-surface rounded-ct-lg p-4 border border-ct-border">
            ...
            {items.map(item => (
              <button
                ...
                className="w-full flex items-center gap-3 py-2.5 border-b border-ct-border/30 last:border-0 active:bg-ct-elevated/30"
              >
                <div role="checkbox" aria-checked={item.isChecked} className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                  item.isChecked ? 'bg-cyan-500 border-cyan-500' : 'border-ct-border'
                }`}>
      ...
      {showAdd ? (
        <div className="bg-ct-surface rounded-ct-lg p-4 border border-ct-border space-y-3">
          <input value={newItem} onChange={e => setNewItem(e.target.value)} placeholder="Item name" aria-label="Item name" className="w-full bg-ct-elevated rounded-xl py-2 px-3 text-ct-1 text-sm focus:outline-none focus:ring-1 focus:ring-cyan-400" autoFocus />
          <div className="flex gap-2">
            <input value={newQty} onChange={e => setNewQty(e.target.value)} placeholder="Qty" aria-label="Quantity" className="flex-1 bg-ct-elevated rounded-xl py-2 px-3 text-ct-1 text-sm focus:outline-none focus:ring-1 focus:ring-cyan-400" />
            <select value={newCategory} onChange={e => setNewCategory(e.target.value)} aria-label="Category" className="bg-ct-elevated rounded-xl py-2 px-3 text-ct-1 text-sm focus:outline-none focus:ring-1 focus:ring-cyan-400">
        ...
        <button onClick={() => setShowAdd(true)} className="w-full bg-ct-surface/40 border border-dashed border-ct-border text-cyan-400 py-3 rounded-ct-lg text-sm font-medium flex items-center justify-center gap-2">
