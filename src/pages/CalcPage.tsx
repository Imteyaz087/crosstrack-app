  ...
  return (
    <div className="space-y-4 stagger-children">
      <div className="flex justify-between items-center">
        ...
        <button onClick={onClose} className="text-ct-2 min-h-[44px] min-w-[44px] flex items-center justify-center" aria-label={t('common.close')}><X size={20} /></button>
      </div>
      ...
      <div className="flex gap-3">
        <div className="flex-1">
          ...
          <input type="text" inputMode="decimal" pattern="[0-9.]*" value={weight} onChange={e => setWeight(e.target.value.replace(/[^\d.]/g, ''))} placeholder="kg"
            className="w-full bg-ct-elevated border border-ct-border rounded-xl py-3 px-4 text-ct-1 text-2xl text-center font-bold focus:outline-none focus:ring-1 focus:ring-cyan-400" />
        </div>
        <div className="flex-1">
          <label className="text-[11px] text-ct-2 uppercase mb-1 block">{t('calc.reps')}</label>
          <input type="text" inputMode="numeric" pattern="[0-9]*" value={reps} onChange={e => setReps(e.target.value.replace(/\D/g, ''))} placeholder="reps"
            className="w-full bg-ct-elevated border border-ct-border rounded-xl py-3 px-4 text-ct-1 text-2xl text-center font-bold focus:outline-none focus:ring-1 focus:ring-cyan-400" />
        </div>
      </div>
        ...
        <>
          ...
          <div className="bg-ct-surface rounded-ct-lg p-4 border border-ct-border">
            ...
            {chart.map(({ pct, weight }) => (
              <div key={pct} className="flex items-center py-1.5 border-b border-ct-border/30 last:border-0">
          ...
          <div className="bg-ct-surface rounded-ct-lg p-4 border border-ct-border">
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              <div className="bg-ct-elevated/40 rounded-lg p-2">
