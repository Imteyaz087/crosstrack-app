...
import { Trash2, Plus, Repeat, Flame } from 'lucide-react'
...
export function NutritionPage() {
  ...
  useEffect(() => {
    Promise.allSettled([loadTodayMeals(), loadTemplates(), loadProfile()])
  }, [])
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-ct-surface rounded-ct-lg p-4 border border-ct-border">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${
            calPct > 100 ? 'bg-red-500/20 text-red-400'
            : calPct > 80 ? 'bg-green-500/20 text-green-400'
      ...
      {mealTypes.map(type => {
        ...
        return (
          <div key={type} className={`bg-ct-surface rounded-ct-lg p-4 border ${hasItems ? 'border-ct-border' : 'border-dashed border-slate-600/40'}`}>
            ...
            {hasItems ? (
              <>
                {meals.map(m => (
                  <div key={m.id} className="flex items-center py-2.5 border-b border-slate-700/30 last:border-0">
                    ...
                    <button onClick={() => m.id && setDeleteTarget({ id: m.id, name: m.foodName })}
                      className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center text-ct-2 active:text-red-400"
                      aria-label={`${t('common.delete')} ${m.foodName}`}>
                      <Trash2 size={16} />
                    </button>
                  </div>
                ))}
              ...
              <div className="flex gap-2 mt-1">
                <button onClick={() => setActiveTab('log')}
                  className="flex-1 bg-slate-700/40 border border-slate-600/50 text-cyan-400 py-2.5 rounded-xl text-xs font-medium flex items-center justify-center gap-1 card-press min-h-[44px]">
                  <Plus size={14} /> {t('nutrition.addMeal')}
                </button>
                {templatesForType.length > 0 && (
                  <button onClick={() => addMealFromTemplate(templatesForType[0])}
                    className="flex-1 bg-slate-700/40 border border-slate-600/50 text-ct-2 py-2.5 rounded-xl text-xs font-medium flex items-center justify-center gap-1 card-press min-h-[44px]">
