    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setShowAdd(!showAdd)}
          className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg active:bg-ct-surface text-cyan-400"
          aria-label={showAdd ? 'Close form' : 'Add new PR'}>
      ...
      {showAdd && (
        <div className="bg-ct-surface rounded-ct-lg p-4 border border-cyan-400/30 space-y-3">
          <input value={movementName} onChange={e => setMovementName(e.target.value)}
            ...
            className="w-full bg-ct-elevated rounded-xl py-2.5 px-3 text-ct-1 text-sm focus:outline-none focus:ring-1 focus:ring-cyan-400 min-h-[44px]"
            autoFocus />

          <div className="flex flex-wrap gap-1.5">
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => setCategory(c)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize min-h-[36px] ${category === c ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-400/40' : 'bg-ct-elevated text-ct-2 border border-slate-600'}`}>
                {c}
              </button>
            ))}
          </div>
          ...
          <div className="flex gap-2 items-center">
            <input type="text" inputMode="decimal" pattern="[0-9.]*"
              ...
              placeholder="Value"
              className="min-w-0 flex-1 bg-ct-elevated rounded-xl py-2.5 px-3 text-ct-1 text-center text-lg font-bold focus:outline-none focus:ring-1 focus:ring-cyan-400 min-h-[44px] tabular-nums" />
            <span className="text-ct-2 text-sm">{unit}</span>
          </div>

          <button onClick={handleSave} disabled={!movementName.trim() || !value}
      ...
      {Object.keys(grouped).length === 0 ? (
        <div className="bg-ct-surface rounded-ct-lg p-6 border border-ct-border text-center">
          <Trophy size={32} className="mx-auto text-ct-2 mb-2" />
          <p className="text-sm text-ct-2">{t('movementPR.noPRsYet')}</p>
          <p className="text-xs text-ct-2">{t('movementPR.noPRsDesc')}</p>
        </div>
      ) : (
        Object.entries(grouped).map(([name, prs]) => (
          <div key={name} className="bg-ct-surface rounded-ct-lg p-4 border border-ct-border">
            ...
            {prs.map(pr => (
              <div key={pr.id} className="flex items-center py-2 border-b border-slate-700/30 last:border-0">
